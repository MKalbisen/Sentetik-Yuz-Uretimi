...
def save_checkpoint(generator, discriminator, optimizer_g, optimizer_d, epoch, losses_g, losses_d, filepath, additional_info=None):
    ...
    checkpoint = {
        'epoch': epoch,
        'generator_state_dict': generator.state_dict(),  #generatör ağırlıkları
        'discriminator_state_dict': discriminator.state_dict(),
        'optimizer_g_state_dict': optimizer_g.state_dict(), #optimizer_g ağırlıkları(learning rate, momentum vs.)
        'optimizer_d_state_dict': optimizer_d.state_dict(),
        'losses_g': losses_g, #generator kayıpları
        'losses_d': losses_d,
        'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S') #kaydedilme zamanı
    }
    
    if additional_info:
        checkpoint.update(additional_info)  #ek bilgiler varsa ekle
    
    torch.save(checkpoint, filepath)  #checkpoint dosyasını kaydet binary olarak
...
def load_checkpoint(filepath, generator, discriminator, optimizer_g, optimizer_d, device):
    ...
    if not os.path.exists(filepath):
        raise FileNotFoundError(f"Checkpoint dosyası bulunamadı: {filepath}") #checkpoint dosyası yoksa hata ver
    
    print(f"Checkpoint yükleniyor: {filepath}")
    checkpoint = torch.load(filepath, map_location=device)  #checkpoint dosyasını yükle
    
    # Model ağırlıklarını yükle
    generator.load_state_dict(checkpoint['generator_state_dict'])  #generator ağırlıklarını ve bias değerlerini yükle
